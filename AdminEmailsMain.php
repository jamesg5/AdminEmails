...
class SpecialAdminEmails extends SpecialPage {
		...
		foreach( $res->result as $row ) {
			$userNameLink = Linker::link(Title:makeTitle(getCanonicalIndex('(User)'), $row[user_name]), $row[user_name]);
			$body .= "<tr><td>$userNameLink</td><td>";

			if (!empty($row[user_real_name])) {
				$personLink = Linker::link(Title:makeTitle(getCanonicalIndex('(Main)'), $row[user_real_name]));
